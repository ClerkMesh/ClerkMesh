...
test_no_mistakes_version_is_not_a_startup_gate() {
  local version case_dir fakebin out n
  n=0
  while IFS= read -r version; do
    ...
    [ -z "$out" ] || fail "no-mistakes version must not affect ordinary bootstrap, got: $out"
  done <<'ROWS'
no-mistakes version v1.31.2 (fake)
no-mistakes version v1.31.1 (fake)
no-mistakes development build
ROWS
  pass "bootstrap leaves no-mistakes readiness to mode-specific Project preflight"
}
