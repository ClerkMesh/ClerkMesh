...
# --- 1. primary-shaped home: a crewmate spawns into the isolated test space ---
...
PRIMARY_TEST_HASH=$(printf '%s' "$TMP_ROOT" | shasum -a 256 | awk '{print substr($1, 1, 12)}')
PRIMARY_TEST_LABEL="firstmate-test-$PRIMARY_TEST_HASH"
[ "$CM1_WS_LABEL" = "$PRIMARY_TEST_LABEL" ] || fail "a primary-shaped home's crewmate should land in the isolated '$PRIMARY_TEST_LABEL' workspace, got '$CM1_WS_LABEL'"
pass "real herdr E2E: the primary-shaped home's crewmate landed in its root-bound workspace"
...
SECONDARY_TEST_LABEL="2ndmate-e2esm1-test-$PRIMARY_TEST_HASH"
[ "$SM_WS_LABEL" = "$SECONDARY_TEST_LABEL" ] || fail "a --secondmate spawn should land in the isolated '$SECONDARY_TEST_LABEL' workspace, got '$SM_WS_LABEL'"
