...
test_fm_home_parameterization() {
  ...
  grep -F "/bin/fm-task-status.sh' --task 'task-a' --state {state}" "$brief" >/dev/null \
    || fail "brief did not use the shell-quoted status boundary"
  ...
  grep -F "/bin/fm-task-status.sh' --task 'task-b' --state {state}" "$brief" >/dev/null \
    || fail "scout brief did not use the shell-quoted status boundary"
  ...
  grep -F "/bin/fm-task-status.sh' --task 'task-c' --state {state}" "$brief" >/dev/null \
    || fail "secondmate brief did not use the shell-quoted status boundary"
