...
STATUS_COMMAND="$(shell_quote "$FM_ROOT/bin/fm-task-status.sh") --task $(shell_quote "$ID")"
...
Report only true captain-relevant outcomes or a declared external wait through Firstmate's status boundary:
   \`$STATUS_COMMAND --state {state} --summary "{one short line}"\`
...
4. Report status through Firstmate's status boundary:
   \`$STATUS_COMMAND --state {state} --summary "{one short line}"\`
   States: working, needs-decision, blocked, $PAUSED_VERB, done, failed.
...
case "$MODE" in
  direct-PR)
    ...
    TOOL_RULE='3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.'
    DOD=$(cat <<EOF
# Definition of done
...
EOF
  ...
  local-only)
    ...
    TOOL_RULE='3. Use chrome-devtools-axi for browser operations. This local-only execution must not use forge tooling.'
    DOD=$(cat <<EOF
# Definition of done
