...
let taskId, type, summary, occurredAt = null;
...
if (!validId.test(taskId ?? "") || !eventTypes.has(type) || typeof summary !== "string" || summary.length < 1 || summary.length > 1000 || /[\u0000-\u001f\u007f]/u.test(summary) || (occurredAt !== null && Number.isNaN(Date.parse(occurredAt)))) fail("fm-task-activity-append: invalid event");
...
try {
  ...
  try {
    ...
    records.forEach((record, index) => {
      ...
      if (keys !== "cursor,observedAt,occurredAt,summary,type" || record.cursor !== index + 1 || !eventTypes.has(record.type) || typeof record.summary !== "string" || record.summary.length < 1 || record.summary.length > 1000 || /[\u0000-\u001f\u007f]/u.test(record.summary) || typeof record.observedAt !== "string" || Number.isNaN(Date.parse(record.observedAt)) || !(record.occurredAt === null || (typeof record.occurredAt === "string" && !Number.isNaN(Date.parse(record.occurredAt))))) throw new Error("record");
    });
  } catch (error) { if (error?.code !== "ENOENT") throw error; }
  const record = { cursor: records.length + 1, type, observedAt: new Date().toISOString(), occurredAt, summary };
