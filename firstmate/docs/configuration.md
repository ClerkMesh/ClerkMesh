...
Required startup tools come in two parts: a common local-control-plane toolchain every home needs regardless of backend, and a per-backend delta that follows the runtime backend actually resolved for this home.
The common startup toolchain is node, git, chrome-devtools-axi, lavish-axi, compatible tasks-axi per "Backlog backend" above, and quota-axi.
This section is the single owner of that startup toolchain list; backend guides' prerequisites point here and add only their backend-specific tools.
Forge delivery dependencies are mode-specific instead: `bin/fm-project-preflight.sh` requires no forge tool for `local-only`, requires an origin plus `gh`, `gh-axi`, and GitHub authentication for `direct-PR`, and additionally requires a passing `no-mistakes doctor` for `no-mistakes`. Ordinary session startup never probes those tools or credentials.
...
Orca provides both the task worktree and terminal endpoint (see "Runtime backend" above), so `backend=orca` requires only `orca` on top of the common startup toolchain and skips both `treehouse` and every other backend's session CLI.
...
FM_WATCH_LIFECYCLE_LOG_MAX_BYTES=262144   # size cap for the Pi extension's own diagnostic lifecycle log
FM_WATCH_LIFECYCLE_LOG_RETAIN_LINES=2000   # newest complete rows kept when that Pi lifecycle log is capped
