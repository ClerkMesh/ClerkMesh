...
import type { FmHerdrAgentsV1, FmTaskGraphV1 } from "./work-types";
import "./styles.css";

const SESSION_PARAM = "session";
const WORKSPACE_PARAM = "workspace";
const TOKEN_KEY = "clerkmesh.conversation-client-token.v1";
type Workspace = "conversations" | "work";
...
function selectedWorkspace(): Workspace { return currentUrl().searchParams.get(WORKSPACE_PARAM) === "work" ? "work" : "conversations"; }
function navigate(workspace: Workspace, session?: string): void { const url = currentUrl(); workspace === "work" ? url.searchParams.set(WORKSPACE_PARAM, "work") : url.searchParams.delete(WORKSPACE_PARAM); if (session) url.searchParams.set(SESSION_PARAM, session); window.history.pushState({}, "", url); window.dispatchEvent(new PopStateEvent("popstate")); }
...
function WorkWorkspace({ tasks, agents, errors }: { tasks: FmTaskGraphV1 | null; agents: FmHerdrAgentsV1 | null; errors: Partial<Record<"tasks" | "agents", WorkError>> }) {
  ...
  return <div className="work-layout"><aside aria-label="Work"><h1>Work</h1><p>Firstmate Task graph and Herdr runtime observations.</p></aside><section aria-live="polite"><p className="crumb">Work / Task graph</p><h2>Task graph</h2>{errors.tasks && <p role="alert">Task graph is stale: {errors.tasks.message} Last checked {observed(errors.tasks.observedAt)}.</p>}{errors.agents && <p role="alert">Agent status is stale: {errors.agents.message} Last checked {observed(errors.agents.observedAt)}.</p>}{!tasks && !errors.tasks && <p>Loading Tasks…</p>}{tasks && <><p className="projection-meta">{tasks.freshness} · observed {observed(tasks.observedAt)}</p><div className="task-list">{tasks.tasks.map((task) => { const agent = agentsByTask.get(task.id); return <article className="task-card" key={task.id}><div><small>{task.projectId}</small><h3>{task.title}</h3><p>{task.phase.replaceAll("_", " ")}{task.wait ? ` · waiting: ${task.wait.reason}` : ""}</p></div><div className="runtime"><strong>{agent?.agent.status ?? task.runtime.state}</strong><span>{agent ? `Herdr · ${observed(agent.agent.observedAt)}` : `${task.runtime.source} · ${observed(task.runtime.observedAt)}`}</span></div></article>; })}{tasks.tasks.length === 0 && <p>No Tasks are currently projected.</p>}</div></>}</section></div>;
}
