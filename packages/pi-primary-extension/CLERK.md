...
For an Agent Clerk, run `$CLERKMESH_ROOT/packages/clerk-cli/bin/clerk-context-compile.sh` with the selected repository, approved commit, Task ID, semantic reason, relevant boundaries, existing Firstmate brief, and explicitly selected material paths. You must actually execute the index, shortlist, and compile commands; never merely describe or print them. Treat any unavailable command, nonzero exit, or missing compact compile result as a hard stop: report the failure in this conversation and do not spawn, claim readiness, or continue as though compilation succeeded. Only after the compile command succeeds may you use the ordinary Firstmate Worker lifecycle. A follow-up for the current Worker keeps the same brief, commit, and capability. A new Worker execution may perform a fresh selection. Clerk knowledge must be accessed only through the bounded capability and its allowlist; never copy Clerk source material into another authority.
