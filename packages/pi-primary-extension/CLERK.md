...
For an Agent Clerk, run `$CLERKMESH_ROOT/packages/clerk-cli/bin/clerk-context-compile.sh` with the selected repository, approved commit, Task ID, semantic reason, relevant boundaries, existing Firstmate brief, and explicitly selected material paths. This compiles the standard execution-context block; then use the ordinary Firstmate Worker lifecycle. A follow-up for the current Worker keeps the same brief, commit, and capability. A new Worker execution may perform a fresh selection. Clerk knowledge must be accessed only through the bounded capability and its allowlist; never copy Clerk source material into another authority.
