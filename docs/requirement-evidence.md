...
| CONV-004 | partial | `test:slice1-schema` binds normalized conversation events to Pi 0.82 RPC event shapes without parsing terminal/ANSI output | Real Pi RPC bridge and CERT-002 remain |
...
| CONV-010 | partial | `test:slice1-schema` proves ordinary snapshots exclude diagnostic events while explicit diagnostic snapshots include them, with defensive payload copies; Pi 0.82 RPC fixtures project visible text/stream/status/extension UI, keep reasoning/tool lifecycles diagnostic-only, and redact credential-shaped fields and Bearer values before storage | UI warning and real diagnostic rendering remain |
