...
| CONV-004 | partial | `test:slice1-schema` binds normalized conversation events to Pi 0.82 RPC event shapes without parsing terminal/ANSI output and proves strict bounded JSONL command correlation/event forwarding fails closed on malformed or unmatched output | Real Pi RPC bridge and CERT-002 remain |
...
| CONV-010 | partial | `test:slice1-schema` proves ordinary snapshots exclude diagnostic events while explicit diagnostic snapshots include them, with defensive payload copies; Pi 0.82 RPC fixtures project visible text/stream/status/extension UI, keep reasoning/tool lifecycles diagnostic-only, redact credential-shaped fields and Bearer values before storage, and require extension command discovery before prompt acceptance | UI warning and real diagnostic rendering remain |
