...
| CONV-009 | partial | `test:slice1-schema` proves a versioned, schema-valid process-local projection retains at most 10,000 normalized events, assigns monotonic sequence cursors, continues after a snapshot cursor, and retains pending extension UI | Snapshot race, Pi-history restart reconstruction, and real refresh remain |
