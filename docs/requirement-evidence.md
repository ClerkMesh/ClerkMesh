...
| CONV-002 | partial | `test:slice1-schema` proves concurrent first sends share one startup promise, resolve opaque session IDs server-side, lock selection once startup begins, and never prompt before startup succeeds | HTTP binding and a real Pi first-message run remain |
...
| CONV-006 | partial | `test:slice1-schema` proves process-local `(client token, requestId)` replay returns the original promise without a duplicate prompt and conflicting reuse fails closed | HTTP request integration and restart-boundary proof remain |
...
| READ-003 | partial | `packages/shared/schemas/conversation-sessions.v1.schema.json`, its reproducibly generated TS type, and the Fastify catalog endpoint's Ajv validation cover the first Web boundary model | All remaining v1 projections remain |
