...
| CONV-007 | partial | `test:slice1-schema` uses a fake clock to prove first-token acquisition, multiple same-token connections, a strict three-second reconnect reservation, second-token refusal, explicit claim after expiry, and vacant new-connection acquisition | HTTP/WebSocket and loopback browser reconnect proof remain |
| CONV-008 | partial | The lease accepts opaque local client tokens only and exposes no actor/authentication semantics | UI wording and transport integration remain |
