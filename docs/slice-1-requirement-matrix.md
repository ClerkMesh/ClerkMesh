...
- Fastify and Ajv are installed for the catalog query. The server pins Pi 0.82.0 and binds discovery to its read-only `SessionManager.listAll()` API; `@fastify/websocket` and `ws` provide the single event/lease channel; React/Vite remain uninstalled.
