...
The first bounded work packages freeze and build the zero-token session catalog crossing the Web boundary. Session identifiers are opaque: canonical session paths remain in a server-only lookup, preventing the browser from supplying filesystem paths. Discovery canonicalizes cwd (including symlink aliases), omits foreign or invalid sessions, rejects ambiguous duplicate IDs, and has no dependency on the Primary launch owner. The Fastify query validates every projection with the frozen schema and returns a path-free generic error when discovery or validation fails. The first-send coordinator resolves that opaque ID server-side, shares one startup promise across concurrent sends, locks selection after startup begins, and deduplicates `(client token, requestId)` within the process. Its HTTP mutation rejects unknown/path-bearing inputs and non-JSON bodies, maps request conflicts to HTTP 409, and returns only after the prompt is accepted.
