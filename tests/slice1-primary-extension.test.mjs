...
for (const rule of [
  ...
  /Firstmate `report\.md` as Markdown with Captain actor provenance.*"type":"captain".*"id":"local"/s,
  /evaluate that report against the Task acceptance criteria before accepting the result/,
  /missing, rejected, or interrupted result remains honestly incomplete.*never infer completion/s,
  /separate dependent Agent Task.*fresh Agent Clerk selection/s,
