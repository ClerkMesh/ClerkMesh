...
for (const rule of [
  ...
  /bundled Research Clerk is a narrow evidence-gathering fallback, not a generalist.*Prefer any active specialist Clerk/s,
  /Select Research only for bounded source discovery, verification, comparison, or reporting.*never use it to implement/s,
  /Project is a durable product\/repository boundary, never a Task, investigation topic, or temporary output folder/,
  /First query Firstmate's authoritative Project catalog.*distinguish an existing Git repository from its registration/s,
  /do not default unrelated work to the ClerkMesh control repository/,
