...
assert.equal(catalog.proposals[0].observedAt, catalog.observedAt);
assert.equal(catalog.proposals[0].freshness, "current");
assert.deepEqual(catalog.proposals[0].provenance, { authority: "clerkmesh-learning-proposals" });
assert.equal(catalog.proposals[0].targets.length, 1, "only materialized reviews are exposed");
assert.deepEqual(catalog.proposals[0].omitted, [{ reason: "Target pending-clerk has no materialized review" }]);
assert.deepEqual(catalog.proposals[0].errors, []);
